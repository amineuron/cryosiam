...
from monai.data import CacheDataset, Dataset, ITKReader, SmartCacheDataset, ThreadDataLoader
...
from cryosiam.utils import supervised_instance_train_val_split
...
class InstanceSegmentationModule(pl.LightningModule):
    def __init__(self, config, backbone_config):
        ...
        self._model = InstanceHeads(n_input_channels=config['parameters']['network']['dense_dim'],
        ...
        self.foreground_loss_function = nn.BCEWithLogitsLoss()
        self.distance_loss_function = nn.MSELoss()
        self.boundaries_loss_function = nn.BCEWithLogitsLoss()
    ...
    def prepare_data(self):
        ...
        if not os.path.isfile(train_val_path):
            train_files, val_files = supervised_instance_train_val_split(data_root, out_root=self.config['temp_dir'],
    ...
    def setup(self, stage=None):
        ...
        train_transforms = Compose([LoadImaged(keys=keys + ['foreground', 'distances', 'boundaries'],
                                    ...
                                    ScaleIntensityRanged(keys=keys, a_min=self.config['parameters']['data']['min'],
                                                         ...
                                                         b_max=1, clip=True),
                                    SpatialPadd(keys=keys + ['foreground', 'distances', 'boundaries'],
                                                spatial_size=self.config['parameters']['data']['patch_size']),
                                    OneOf(transforms=image_transforms),
                                    RandZoomd(keys=keys + ['foreground', 'distances', 'boundaries'], prob=0.8,
                                    ...
                                    EnsureTyped(keys=keys + ['distances', 'foreground', 'boundaries'],
    ...
    def training_step(self, batch, batch_idx):
        inputs, foreground, distance, boundaries = batch['image'], batch['foreground'], batch['distances'], \
            batch['boundaries']
        if self.use_noisy_input:
            inputs = batch[random.choices(['image', 'noisy_image'], weights=[2, 1], k=1)[0]]
        foreground_pred, distance_pred, boundaries_pred = self.forward(inputs)
        f_loss = self.foreground_loss_function(foreground_pred, foreground)
        d_loss = self.distance_loss_function(distance_pred, distance)
        b_loss = self.boundaries_loss_function(boundaries_pred, boundaries)
        loss = f_loss + d_loss + b_loss
        self.log('train_loss', loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        self.log('train_foreground_loss', f_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        self.log('train_distance_loss', d_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        self.log('train_boundary_loss', b_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        b_loss = self.boundaries_loss_function(boundaries_pred, boundaries)
        loss = f_loss + d_loss + b_loss
        self.log('val_loss', loss, on_step=False, on_epoch=True, batch_size=self.batch_size, sync_dist=self.sync_dist)
        self.log('val_foreground_loss', f_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        self.log('val_distance_loss', d_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
        self.log('val_boundary_loss', b_loss, on_step=False, on_epoch=True, batch_size=self.batch_size,
                 sync_dist=self.sync_dist)
