...
def main(coordinates_file, sphere_radiuses, output_dir, example_tomogram, tomo_name):
    ...
    for t_name in np.unique(data[header_name]):
        ...
        for class_num in np.unique(current_data['rlnClassNum'] if file_type == 0 else current_data['semantic_class']):
            if file_type == 0:
                current_data_current_class = current_data[current_data['rlnClassNum'] == class_num]
            else:
            ...
            for i, row in current_data_current_class.iterrows():
                if file_type == 0:
...
def parser_helper(description=None):
    description = "Create multiclass tomogram mask with sphere map from given center coordinates" if description is None else description
    ...
    parser.add_argument('--sphere_radius', type=str, required=True,
